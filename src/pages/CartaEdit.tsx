...
const CartaEdit: React.FC = () => {
    ...
    const handleSelect = async (event:any) => {
        setMazo(event)
        if(user){ 
            const q = query(collection(db,"ColeccionMazos",mazo,"Cartas",));
            const unsub = onSnapshot(q, (querySnapshot) => {
              querySnapshot.docChanges().forEach((change) => {
                if (change.type === "added") {
                    if(indexer.indexOf(change.doc.id) === -1){ 
    ...
    return(
      ...
      <IonContent fullscreen color={'medium'}>
        <IonItem className='mazoSelectContainer'>
            <IonSelect cancelText='Cancelar' interface='action-sheet' placeholder="Seleccione mazo para comenzar" interfaceOptions={options}  onIonChange={(e) => handleSelect(e.detail.value)}>
        ...
        <IonItem className='mazoSelectContainer'>
            <IonSelect cancelText='Cancelar' interface='action-sheet' placeholder="Seleccione mazo para comenzar" interfaceOptions={options}  onIonChange={(e) => handleCardSelect(e.detail.value)}>
